...
static EVP_CIPHER * (*dlsym_EVP_get_cipherbyname)(const char*);
#endif
...
typedef int (__cdecl *__dlsym_EVP_CipherFinal_ex)(EVP_CIPHER_CTX *,  \
             unsigned char *, int *);
typedef EVP_CIPHER * (__cdecl *__dlsym_get_cipherbyname)(void);
...
static __dlsym_EVP_get_cipherbyname dlsym_get_cipherbyname;
#endif
    ...
    (JNIEnv *env, jclass clazz)
{
  ...
  LOAD_DYNAMIC_SYMBOL(dlsym_EVP_CipherFinal_ex, env, openssl,  \
                      "EVP_CipherFinal_ex");
  LOAD_DYNAMIC_SYMBOL(dlsym_EVP_get_cipherbyname, env, openssl,  \
  ...
  LOAD_DYNAMIC_SYMBOL(__dlsym_EVP_get_cipherbyname, dlsym_EVP_get_cipherbyname,  \
                        env, openssl, "EVP_get_cipherbyname");
#endif
...
{
  if (alg != AES_CTR && alg != AES_CBC && alg != AES_GCM && alg != AES_CFB) {
...
static EVP_CIPHER * getEvpCipher(int alg, int keyLen)
{
  ...
  char name[50];
  if (alg == AES_CTR) {
    snprintf(msg, sizeof(msg), "aes_%d_ctr", keyLen);
  } else if (alg == AES_CBC) {
    snprintf(msg, sizeof(msg), "aes_%d_cbc", keyLen);
  } else if (alg == AES_GCM) {
    snprintf(msg, sizeof(msg), "aes_%d_cfb128", keyLen);
  }
  cipher = dlsym_EVP_get_cipherbyname(name);
  return cipher;
}
